function [coste, ruta]=aestrella(grafo, heuristica,  inicio, destino)

nnodos = size(grafo,1); %número de nodos
nodos_a_revisar = Inf(nnodos, 3); %nodos sin revisar. Cada fila corresponde a un nodo, por orden, y las columnas son el coste acumulado y el nodo del que viene
nodos_revisados = Inf(nnodos, 4);
nodo_actual = inicio;
nodos_a_revisar(inicio,1) = 0;
nodos_a_revisar(inicio,2) = inicio;
nodos_a_revisar(inicio,3) = heuristica(inicio);
...
%comprobación de funcionamiento más óptimo q dijkstra
nodos_expandidos = 0;
...
while(fin == 0)
    ...
    for m = 1:1:size(array_adyac)
        ...
        heuristica_acum_nodo_adyac = nodos_a_revisar(array_adyac(m),2);
        ...
        nuevo_coste_heuristica = nuevo_coste + heuristica(array_adyac(m), destino);
        if ( coste_acum_nodo_adyac >  nuevo_coste) %el valor de la heuristica se mantiene independientemente del camino, lo q importa es esta condicion
            nodos_a_revisar(array_adyac(m),1) = nuevo_coste; %actualizamos la tabla
            nodos_a_revisar(array_adyac(m),2) = nuevo_coste_heuristica; %actualizamos la heuristica
            nodos_a_revisar(array_adyac(m),3) = nodo_actual; %ponemos de donde sale el coste
        end
    end
    %paso 3: marcamos el nodo como revisado
    nodos_expandidos = nodos_expandidos + 1;
    nodos_revisados(k,:) = [nodo_actual,nodos_a_revisar(nodo_actual,1), nodos_a_revisar(nodo_actual,2),nodos_a_revisar(nodo_actual,3)];%el nodo que era, su coste, y de qué nodo venía
    k = k+1;
    ...
    if ~any(validos)
        ...
        %esta vez clasificamos en funcion de la heuristica
        [~, posicion_min] = min(tabla_temporal(:,2)); %el primer valor q devuelve es el valor mas pequeño, el segundo la posic
        ...
        if nodo_actual == destino
            fin = 1;
            nodos_revisados(k,:) = [nodo_actual,nodos_a_revisar(nodo_actual,1), nodos_a_revisar(nodo_actual,2),nodos_a_revisar(nodo_actual,3)];
        end
    end
...
while nodo ~= inicio
    ...
    nodo = nodos_revisados(fila, 4); % nodo del que viene
    ruta = [nodo, ruta];
end
