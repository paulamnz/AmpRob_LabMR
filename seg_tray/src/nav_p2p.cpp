...
private:
    ...
    void controlLoop()
    {
        ...
        // Coordenadas locales:
        double x_local = cos(current_theta_)*(goal_x_ - current_x_) + sin(current_theta_)*(goal_y_ - current_y_);
        double y_local = -sin(current_theta_)*(goal_x_ - current_x_)+ cos(current_theta_)*(goal_y_ - current_y_);
        double distance = sqrt(x_local * x_local + y_local * y_local);

        //Error de ángulo
        double theta_error = atan2(y_local,x_local);
        ...
        //Curvatura
        double gamma = control_gain_*0.5*theta_error;
        //el aumento de la ganancia hace que aumente gamma, que a su vez hace que aumente la
        //la diferencia de velocidad angular a la que giran las ruedas
        //y por tanto, produce un aumento de la velocidad angular del robot
        ...
        if (abs(angular_velocity) > max_angular_speed_)
        {
